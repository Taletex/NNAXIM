...
/* roundb(f, 18) => keep 18 bits in the float, set the other bits to zero */
...
/* If a file containing the model of the network exists the function loads the network from this file, else creates a new model for the network. Finally it returns the network */
...
/* Approximates the weights of the network passed as argument according to the selected configuration */
...
/* Returns the number of bits saved thanks to the approximation applied on the weights bit size */
...
void train_network(tiny_dnn::network<tiny_dnn::sequential> net, char config, bool exec_ac, vector<tiny_dnn::vec_t> p_train_images, vector<tiny_dnn::label_t> p_train_labels, vector<tiny_dnn::vec_t> p_test_images, vector<tiny_dnn::label_t> p_test_labels) {
	...
	size_t batch_size = (config == CONFIG::BASE) ? BASE_BATCH_NUMBER : RETRAIN_BATCH_NUMBER;  // n samples for each network weight update
	int epochs = (config == CONFIG::BASE) ? BASE_EPOCHS_NUMBER : RETRAIN_EPOCHS_NUMBER;			  // m presentation of all samples
	...
	tiny_dnn::timer t;
	optimizer.alpha *= static_cast<tiny_dnn::float_t>(sqrt(batch_size) * LEARNING_RATE);
	...
	// learn
	net.train<tiny_dnn::cross_entropy>(optimizer, (dataset_yet_loaded ? p_train_images : train_images), (dataset_yet_loaded ? p_train_labels : train_labels), batch_size, epochs, on_enumerate_minibatch, on_enumerate_epoch);
...
/* Inizialize the model and weights of the network on its base configuration (no approximation) */
...
/* Tests all approximate network configurations */
void automatic_test() {
	...
	for (int i = 0; i < (sizeof(configs) / sizeof(*configs)); i++) {
		...
		accuracy_before_retrain[i] = test_network(net, configs[i], false, test_images, test_labels);
	}

	cout << endl << "> Test automatico terminato!" << endl;
	cout << "> Risultati del test: " << endl;
	cout << "   - CONFIGURAZIONE 0: " << (accuracy_before_retrain[9] * 100) << "%" << endl;
	for (int i = 0; i < (sizeof(configs) / sizeof(*configs)); i++) {
		cout << "   - CONFIGURAZIONE " << string(1, configs[i]) << ": " << (accuracy_before_retrain[i] * 100) << "%" << endl;
	}
	cout << endl;
}
...
void ac_nn_test() {
	...
	cout << "> Verra' testata la configurazione originale. Dunque per ogni configurazione saranno effettuati (a partire dai pesi originali): approssimazione, test, training, test." << endl;
